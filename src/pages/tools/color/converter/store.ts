...
import type { ColorPalette } from '~/utils/color'

import { transforms as colorTransformers } from '~/utils/color'
...
const colorTransform = <T extends ColorPalette>(key: T, val: string) => {
  ...
  return needTransfrom.reduce(
    (acc: any, cur: string) => {
      let nextValue = val
      if (key !== 'rgb') {
        nextValue = (colorTransformers as any)[camelCase(`${key}-to-rgb`)]?.(
          val,
        )
      }
